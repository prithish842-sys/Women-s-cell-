  ...
  return (
    <div className="space-y-5 fade-in-up">
      <section className="flex flex-col gap-4 rounded-[20px] bg-[linear-gradient(110deg,#eef3ff,#f4f1ff)] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7">
        <div>
          <h1 className="text-3xl font-black tracking-[-0.03em] text-[#071426]">Notifications Center</h1>
          <p className="mt-1 text-xs font-semibold text-[#52617f]">Student in-charge updates and internal alerts for faculty reviewers.</p>
        </div>
      ...
      {loading ? <NotificationListSkeleton /> : error ? <p className="text-red-600">{error}</p> : notifications.length === 0 ? (
        ...
        <div className="divide-y rounded-xl border border-[#e4eaff] bg-white shadow-sm">
